...
class myBts {
	...
	function viewBts($msisdn){
		$sql = "SELECT b.bts_name, b.ssa_id,b.bts_type,b.bts_id FROM mybts a INNER JOIN ntmsdb.m_bts_master b ON a.bts_id=b.bts_id 
	...
	function getUserMyBtsCount($user_privs, $circle_id){
		if($user_privs=='co'){
			$sql = "SELECT coalesce(circle_id,'TOT') circle, COUNT(msisdn) total_users , COUNT(mybts_msisdn) mybts_cnt,
			...
			$sql = "SELECT circle_id circle, COUNT(msisdn) total_users , COUNT(mybts_msisdn) mybts_cnt,
				sum(case when last_login>DATE(SYSDATE()) then 1 ELSE 0 END) login_count FROM( 
				SELECT aa.circle_id,aa.msisdn,ab.msisdn mybts_msisdn,aa.last_login  FROM cnmcmob.app_users aa LEFT JOIN 
				(SELECT distinct a.circle_id, b.msisdn FROM ntmsdb.m_bts_master a INNER JOIN cnmcmob.mybts b
	...
	function getCircleUsersList($circle_id){
		$sql = "SELECT msisdn, name, desg,email, hrms_no, ssaname, circle, circle_id,last_login, app_version,lvl, lvl2,lvl3
		from cnmcmob.app_users WHERE  circle_id like '$circle_id' AND STATUS='Y' ORDER BY 1";
		$res = mysqli_query($this->conn, $sql);
		while($row[] = mysqli_fetch_assoc($res));
		return array_filter($row);
	}

	function getCircleMyBtsUsersList($circle){
		$sql = "SELECT msisdn, name, circle, desg, email, COUNT(msisdn) cnt FROM (
			...
			ON a.msisdn=b.msisdn  AND a.status='Y' AND b.status=0 AND a.circle_id='$circle') aa
	...
	function getBtsSSA($ssa_id){
		$sql ="select bts_name, bts_id from ntmsdb.m_bts_master where ssa_id='$ssa_id' and e_p<>'P' order by 1";
		$res = mysqli_query($this->conn, $sql);
		while($row[] = mysqli_fetch_assoc($res));
		return array_filter($row);
